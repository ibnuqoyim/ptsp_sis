...
<table class="table table-condensed table-bordered table-striped">
    <thead>
        <tr class="info">
            ...
            <th>Nama Penerima</th>
            <?if(!$this->session->flashdata('cetak') && $this->uri->segment(2)=="orderSerahTerima" ){?>
...
foreach($resultOrderSertifikat as $row){
    ...
    <? if($this->uri->segment(2)=="orderSerahTerima" ) { ?>
        <td colspan="2">
                <div>
                <?=anchor('order/orderSerahTerimaProses/'.@$row->kd_order_sertifikasi.'/'.@$row->kd_sertifikat.'/proses',
