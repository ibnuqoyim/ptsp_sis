...
<table class="table table-condensed table-bordered table-striped">
    <thead>
        <tr class="info">
            ...
            <?if(!$this->session->flashdata('cetak') && $this->uri->segment(2)=="orderPenawaran" ){?>
...
foreach($result as $row){
	...
	<? if($this->uri->segment(2)=="orderPenawaran" ) { ?>
		<td colspan="2">
				<div>
				<!--
				<?=anchor('komoditi/index/'.$row->kd_penawaran."/-/-/edit",
					img(array('src'=>'images/edit.gif','border'=>'0','height'=>'16')),
					array('title'=>'Edit','alt'=>'Edit'));?>&nbsp;&nbsp;|&nbsp;&nbsp;-->
				<?=anchor('order/delOrderPenawaran/'.@$row->kd_order_sertifikasi.'/'.@$row->kd_penawaran,
				img(array('src'=>'images/del.gif','border'=>'0','height'=>'16')),
				array('title'=>'Hapus','alt'=>'Hapus',
				'onclick'=>'return confirm(\'Yakin '.$row->kd_penawaran.' ingin dihapus?\')'));?>
				</div>
		</td>
	<? }?> 
