...
class CI_SpreadSheetCell  {
	...
	public function __construct($sheetid='', $coord=array(), $prefix = '') {
		$this->sheetid;
		$this->prefix;
		$coords = $this->parse_coord($this->coord);
		if ($coords === false) {
			$this->incorrect = true;
			return false;
		}
	...
	public function exists($dont_create = false) {
		$query = "SELECT `data` FROM {$this->prefix}order_data_parameter_pengujian WHERE `sheetid`='{$this->sheetid}' AND `rowid`='{$this->row}' AND `columnid`='{$this->col}'";
		$result = $this->db->query($query);
		if ($this->db->row_array($result))
			return true;
		else {
			if (!$dont_create) {
				$query = "INSERT INTO `{$this->prefix}order_data_parameter_pengujian` (`sheetid`, `rowid`, `columnid`) VALUES ('{$this->sheetid}', '{$this->row}', '{$this->col}')";
				$this->db->query($query);
			}
	...
	public function setValue($text) {
		$this->exists();
		$query = "UPDATE order_data_parameter_pengujian SET `data`='{$text}' WHERE `sheetid`='{$this->sheetid}' AND `rowid`='{$this->row}' AND `columnid`='{$this->col}'";
		$result = $this->db->query($query);
		return $result;
	}
	...
	public function getCalculatedValue() {
		$query = "SELECT `calc` FROM {$this->prefix}order_data_parameter_pengujian WHERE `sheetid`='{$this->sheetid}' AND `rowid`='{$this->row}' AND `columnid`='{$this->col}'";
		$result = $this->db->query($query);
		if (!$result) return false;
		$cell = $this->db->row_array($result);
		if ($cell['calc'] === '')
	...
	public function getParsedValue() {
		$query = "SELECT `parsed` FROM {$this->prefix}order_data_parameter_pengujian WHERE `sheetid`='{$this->sheetid}' AND `rowid`='{$this->row}' AND `columnid`='{$this->col}'";
		$result = $this->db->query($query);
		if (!$result) return false;
		$cell = $this->db->row_array($result);
		return $cell['parsed'];
	...
	public function getStyle() {
		$query = "SELECT `style` FROM {$this->prefix}order_data_parameter_pengujian WHERE `sheetid`='{$this->sheetid}' AND `rowid`='{$this->row}' AND `columnid`='{$this->col}'";
		$result = $this->db->query($query);
		if (!$result) return false;
		$cell = $this->db->row_array($result);
	...
	public function setStyle($style) {
		...
		$query = "UPDATE {$this->prefix}order_data_parameter_pengujian SET `style`='{$style}' WHERE `sheetid`='{$this->sheetid}' AND `rowid`='{$this->row}' AND `columnid`='{$this->col}'";
		$result = $this->db->query($query);
		return $result;
	}
