...
public function Add($perusahaan_pemohon='',$perusahaan_importir='',$kd_sertifikasi_jenis=''){
		if($this->session->userdata('profil')->groupname == 'customer'){
			$perusahaan_pemohon = $this->session->userdata('userid');
		}
		$this->errormsg="";
	...
	public function edit($kd_order_sertifikasi,$perusahaan_pemohon='',$perusahaan_importir='',$kd_sertifikasi_jenis=''){
	 ...
	 public function orderPabrik($kd_order_sertifikasi='',$kd_order_pabrik=''){
	    ...
	    if($this->session->userdata('profil')->groupname == 'customer'){
			$kd_order_pabrik = $this->session->userdata('userid');
		}
		if($kd_order_pabrik){
	...
	*/
	...
	public function kirimDataOrderStepBerikutnya($kd_order_sertifikasi,$kd_step_status='',$next_step=''){
		...
		$toview['next_step']= $next_step;
		if($this->session->userdata('profil')->groupname == 'customer'){
			$toview['nip'] = $this->session->userdata('userid');
		} else {
			$toview['nip']=$user->nip_baru;	
		}
	...
	public function orderDokumen($kd_order_sertifikasi='',$kd_sertifikasi_jenis='',$kd_sertifikasi_jenistarif=''){	
			...
			if($order_dokumen){
				$kd_order_sertifikasi_dokumen = $order_dokumen->kd_order_sertifikasi_dokumen;
		 	...
		 	if($order_dokumen)
				...
				print $order_dokumen->kd_order_sertifikasi_dokumen;
	   		...
	   		$resultdok= $this->mDokumen->getDokumen('',$kd_sertifikasi_jenistarif ,true);
	   				//echo count($result2);
			if($resultdok){
				//print $order->kd_order_sertifikasi;
				//print $order_dokumen;
				if ($kd_sertifikasi_jenis=='bpkimi14-jns-2' && $kd_sertifikasi_jenistarif=='bpkimi14-jnt-4'){
			  		$this->listDokumen = '<table><tr><th colspan="4" style="text-align:center;"><h4>Importir</h4></th></tr>';
					$this->listDokumen .= '<tr><th style="text-align:center;">No</th><th style="text-align:center;">Nama dokumen</th><th style="text-align:center;padding: 0 10px;">Status</th><th style="text-align:center;">File</th></tr>';
					...
					$no = 1;
			      	foreach($resultdok as $row){
			      		if($row->jenis_dokumen=='Importir'){
							...
							$this->listDokumen .= '<tr><td>'.$no.'.&nbsp;&nbsp;</td>';
							if($this->session->userdata('profil')->groupname == 'customer'){
								$this->listDokumen .= '<td>'.ucfirst($row->nama_sertifikasi_dokumen).'</td>';
								if ($ceklist == "checked"){
									$this->listDokumen .= "<td>Terverifikasi</td>";
								} else {
									$this->listDokumen .= '<td style="padding: 0 10px;">'.$row->status_sertifikasi_dokumen.'</td><td>'.form_open_multipart('order/simpanDokumen').'<input type="hidden" name="kd_sertifikasi_dokumen" '.$ceklist.' id="dokumen-'.$counter.'" 
								value="'.$row->kd_sertifikasi_dokumen.'"><input type="hidden" name="kd_order_sertifikasi" value="'.$kd_order_sertifikasi.'"/><input type="file" name="file" onchange="this.form.submit()"/><button type="submit" class="btn-upload" style="display:none;">Upload</button>'.form_close().'</td>';
								}
							} else {
								$this->listDokumen .= '<td>'.ucfirst($row->nama_sertifikasi_dokumen).'</td>';
								
								$this->listDokumen .= '<td style="text-align:center;"><input type="checkbox" name="dokumen['.$counter.']" '.$ceklist.' id="dokumen-'.$counter.'" 
								value="'.$row->kd_sertifikasi_dokumen.'"></td>';
								$this->listDokumen .= '<td style="text-align:center;">-</td>';
							}
							$this->listDokumen .= '</tr>';
							$counter++; $no++;
						}
					...
					$this->listDokumen.='</table>';
					
					$this->listDokumen1='<table><tr><th colspan="4" style="text-align:center;"><h4>Manufaktur</h4></th></tr>';
					$this->listDokumen1 .= '<tr><th style="text-align:center;">No</th><th style="text-align:center;">Nama dokumen</th><th style="text-align:center;padding: 0 10px;">Status</th><th style="text-align:center;">File</th></tr>';
					...
					$no = 1;
			      	foreach($resultdok as $row){
			      		if($row->jenis_dokumen=='Manufaktur'){
							...
							$this->listDokumen1 .= '<tr><td>'.$no.'.&nbsp;&nbsp;</td>';
							if($this->session->userdata('profil')->groupname == 'customer'){
								$this->listDokumen1 .= '<td>'.ucfirst($row->nama_sertifikasi_dokumen).'</td>';
								if ($ceklist == "checked"){
									$this->listDokumen1 .= "<td>Terverifikasi</td>";
								} else {
									$this->listDokumen1 .= '<td style="padding: 0 10px;">'.$row->status_sertifikasi_dokumen.'</td><td>'.form_open_multipart('order/simpanDokumen').'<input type="hidden" name="kd_sertifikasi_dokumen" '.$ceklist.' id="dokumen-'.$counter.'" 
								value="'.$row->kd_sertifikasi_dokumen.'"><input type="hidden" name="kd_order_sertifikasi" value="'.$kd_order_sertifikasi.'"/><input type="file" name="file" onchange="this.form.submit()"/><button type="submit" class="btn-upload" style="display:none;">Upload</button>'.form_close().'</td>';
								}
							} else {
								$this->listDokumen1 .= '<td>'.ucfirst($row->nama_sertifikasi_dokumen).'</td>';
								
								$this->listDokumen1 .= '<td style="text-align:center;"><input type="checkbox" name="dokumen['.$counter.']" '.$ceklist.' id="dokumen-'.$counter.'" 
								value="'.$row->kd_sertifikasi_dokumen.'"></td>';
								$this->listDokumen1 .= '<td style="text-align:center;">-</td>';
							}
							$this->listDokumen1 .= '</tr>';
							$counter++;
							$no++;
						}
					}
					...
					$this->listDokumen1.='</table>';
					if($this->session->userdata('profil')->groupname != 'customer'){
							$this->listDokumen1.= '</br><input type="checkbox" name="cekAllDokumen" value="1" id="cekAllDokumen" 
			...
			if($this->input->post('simpan')){
				echo "<script>alert('Gagal');</script>";
								...
								if($this->errormsg=="") { 
									...
									$config['upload_path']   = './uploads/dokumen_sertifikasi'; 
									$config['allowed_types'] = 'doc|pdf|docx|jpg|png|jpeg|txt'; 
									//$new_name = $this->input->post('kd_order_sertifikasi').$this->input->post('kd_sertifikasi_jenistarif');
									//$config['file_name'] = $new_name;
									$this->load->library('upload', $config);
									
									$dokumenlist=$this->input->post('dokumen');
									//$filelist=$this->input->post('file');
									
									if ($this->upload->do_upload('file')) {
										echo "<script>alert('Berhasil');</script>";
										$data = array('upload_data' => $this->upload->data());
										//$this->load->view('upload_success', $data);
										$this->content=$this->load->view('order/sertifikasi/order_dokumen',$data);	
									} else {
										echo "<script>alert('Gagal');</script>";
										$data = "TINSINT";
									}
	...
	public function simpanDokumen(){
		...
		$file_dir='./uploads/dokumen_sertifikasi/';
		$config['upload_path']          = $file_dir;
		$config['allowed_types']        = 'doc|pdf|docx|jpg|jpeg|png';
		$config['overwrite'] 			= TRUE;
		$config['file_name'] 			= $this->input->post('kd_order_sertifikasi')."_".$this->input->post('kd_sertifikasi_dokumen');
	 	
		$this->load->library('upload', $config);
		
		// upload file, handle it if success or failed
		if (!$this->upload->do_upload('file')){
			// upload failed, send error message
			...
			print "YEAY";
			// upload success, save document name (including its path) to db
			$data = array('upload_data' => $this->upload->data());
			$file_name = $file_dir.$data['upload_data']['file_name'];
			$kd_order_sertifikasi = $this->input->post('kd_order_sertifikasi');
			$kd_sertifikasi_dokumen = $this->input->post('kd_sertifikasi_dokumen');
			
			// check dulu, sebelumnya udh pernah upload apa belum, kalo belum bikin kd_order_sertifikasi_dokumen dulu
			///TODO
			$kd_order_sertifikasi_dokumen = $this->mOrder->Make_kd_order_sertifikasi_dokumen();
			
			//save document name to db
			$hasil = $this->mOrder->saveOrderDokumen($kd_order_sertifikasi, $kd_sertifikasi_dokumen, $file_name, $kd_order_sertifikasi_dokumen);
			
			//redirect
			if ($hasil) {
				echo "<script>alert('Penyimpanan berhasil');</script>";
				redirect('order/orderDokumen/'.$kd_order_sertifikasi);
			...
			/*
			//$toview['file_sertifikasi_dokumen']=
			
			if ($hasil) {
				$this->errormsg='<em style="color:green">Berhasil di simpan!</em>';
				redirect('order/orderDokumen/'.$toview['kd_order_sertifikasi']);
			...
			//echo "<script>alert('".$data['upload_path'].$data['file_name']."');</script>";
			
			//echo "<script>alert('".$this->input->post('kd_order_sertifikasi')."');</script>";
			//echo "<script>alert('".$this->input->post('kd_sertifikasi_dokumen')."');</script>";
			//$this->mOrder->simpanDokumen($data['file_name'], $this->input->post('kd_order_sertifikasi'), $this->input->post('kd_sertifikasi_dokumen'));
			*/
		}
		...
		/*if (! $this->upload->do_upload('file'))
						{
				    		...
				    		//echo "<script>alert('test no error message true')</script>";\
							$data = array('upload_data' => $this->upload->data());
							$toview['file']=$data['upload_data']['file_name'];			    		
									
							$hasil=$this->mOrder->saveOrderDokumen($toview,$toview['kd_order_sertifikasi'],false); 
							//echo "<script>alert('test disimpan')</script>";
							if($hasil) { 
								//$this->mUser->WriteLog($this->session->userdata('userid'),$_SERVER['REMOTE_ADDR'],
								...
								//echo "<script>alert('Maaf, Penyimpanan gagal boss!')</script>";
							redirect(current_url()); }
		//redirect('order/orderDokumen/'.$this->input->post('kd_order_sertifikasi'));
	}*/
	}
	...
	public function orderSertifikasiStatus($kd_order_sertifikasi){
		...
		if($this->session->userdata('profil')->groupname != 'customer'){
			$toview['rincian_historis'] .= "<td class=\"c-table-x\">By</td>";
		}
		$toview['rincian_historis'] .=	"</tr>";		
		...
		if($historis_status_wo){
			...
			foreach($historis_status_wo as $row){
				...
				if($this->session->userdata('profil')->groupname != 'customer'){
					$toview['rincian_historis'] .= "<td class=\"c-clas-text-1\">".$user->groupdesc." (".$row->nip.")</td>";
				}
