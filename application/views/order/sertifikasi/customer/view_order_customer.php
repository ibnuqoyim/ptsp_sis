...
<p>&nbsp;</P>
<h3 style="text-align:center;">Daftar Order
<?php 
	...
	$tombol2='<span style="float:right"><img src="images/printer.gif" border="0" height="28"><br />
		<span class="textkolom">Cetak</span></span>';
	$tombol3='<span style="float:right"><img src="images/xls.gif" border="0" height="28" /><br />
		<span class="textkolom">Excel </span></span>';
	//-----------------------------------------------------------//
	//------Tombol Tambah order pada menu Order------------------//
	//-----------------------------------------------------------//
	echo anchor('order/Add/',$tombol,'style="text-decoration:none;float:right;"');
	//echo anchor('order/cetak/'.base64_encode(current_url()),$tombol2,'target="_blank" style="text-decoration:none"');
	//echo anchor('order/excels/'.base64_encode(current_url()).'/list_order_pengujian',$tombol3,
		...
		<td><div>
		 ...
		 <td>
		  ...
		  <!--<?//=anchor('order/view/'.$row->kd_order_sertifikasi,img(array('src'=>'images/preview.gif','border'=>'0','height'=>'16')),
		   ...
		   <? if($row->status_order_sertifikasi <> "Closed") { ?>
		   |		
		   <?=anchor('order/delOrder/'.$row->kd_order_sertifikasi.'/0/1',img(array('src'=>'images/del.gif','border'=>'0','height'=>'16')),
		   array('title'=>'Hapus','alt'=>'Hapus','onclick'=>'return confirm(\'Yakin No. Order '.
			$row->noreg_order_sertifikasi.' ingin dihapus bersama turunannya?\')'));?>   
		   <?php }?>                                       
		  </div>
		</td>
	  </tr>
