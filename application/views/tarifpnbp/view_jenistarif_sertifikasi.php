...
<ul class="breadcrumb">
    ...
    <li class="active"><?=$nama_sertifikasi_jenis;?></li>
</ul>
...
<p><?php 
	$tombol='<button type="button" style="border:none;cursor:pointer;background-color:transparent" id="addbutton">
		...
		echo anchor('tarif/JenisTarif/'.$this->uri->segment(3).'/-/-/add/',$tombol,'style="text-decoration:none"');
?></p>
<?if(!empty($result)){?>
...
?>
	<tr>
		...
		<td>
			...
			<?=anchor('tarif/InputItem/'.$row->kd_sertifikasi_jenistarif,
				img(array('src'=>'images/tambah.png','border'=>'0','height'=>'20')),
			array('title'=>'Input Jenis Item','alt'=>'Input Jenis Item'));?> 
			<?=anchor('tarif/InputItem/'.$row->kd_sertifikasi_jenistarif,$row->nama_jenistarif,
			array('title'=>'Input Jenis Item','alt'=>'Input Jenis Item'));?></div></td>
		
		<td>
			<div class="c-clas-text-2">
				<?=anchor('tarif/InputParameter/'.$row->kd_sertifikasi_jenistarif,
						img(array('src'=>'images/tambah.png','border'=>'0','height'=>'20')),
				        array('title'=>'Input Parameter','alt'=>'Input Parameter'));?>&nbsp;&nbsp;|&nbsp;&nbsp;
				<?=anchor('tarif/JenisTarif/'.$row->kd_sertifikasi_jenis.'/'.$row->kd_sertifikasi_jenistarif.'/-/edit',
						img(array('src'=>'images/edit.gif','border'=>'0','height'=>'16')),
						array('title'=>'Edit','alt'=>'Edit'));?>&nbsp;&nbsp;|&nbsp;&nbsp;
				<?=anchor('tarif/JenisTarif/'.$row->kd_sertifikasi_jenis.'/'.$row->kd_sertifikasi_jenistarif.'/-/del',
						img(array('src'=>'images/del.gif','border'=>'0','height'=>'16')),
						array('title'=>'Hapus','alt'=>'Hapus','onclick'=>'return confirm(\'Yakin '.$row->nama_jenistarif.' ingin dihapus berikut turunannya?\')'));?>
			</div></td>
	</tr>
<?}?>
</table>
